...
Omitting `gateway_jwt.ttl_secs` selects non-expiring launch-scoped gateway and
Sandbox Protocol tokens for local single-player Docker, Podman, and VM
gateways; both token profiles carry `exp = 0`. Kubernetes and other shared
deployments should set a positive TTL. Explicit zero is rejected.
